  ...
  return (
    <GuideShell slug="motion-language">
      <JsonLd
        data={[
          articleLd({
            headline: "The Motion Language",
            description:
              "Five springs, one tween scale, and when to use which.",
            path: "/guides/motion-language",
            serial: guideBySlug("motion-language")?.serial,
          }),
          breadcrumbLd([
            { name: "Home", path: "/" },
            { name: "Guides", path: "/guides" },
            { name: "The Motion Language", path: "/guides/motion-language" },
          ]),
        ]}
      />
      <GuideSection title="Five springs, deliberately few">
        <GuideP>
          ...
          ratio — the number that defines its personality — and every component
          in the library draws from the same set. That constraint is the point:
          when everything speaks the same physics, compositions feel machined
          from one piece.
        </GuideP>
        <CalibrationStrip />
        <GuideTable
          ...
          rows={[
            [
              ...
              "0.99",
              "~120ms",
              "Press states, tick draws, focus. Confirms.",
            ],
            [
              "snap",
              "0.83",
              "~300ms",
              "Toggles, tabs, menus — one crisp overshoot. Switches.",
            ],
            [
              "glide",
              "0.98",
              "~450ms",
              "Layout shifts, reorders, morphs. Moves.",
            ],
            [
              "drift",
              "1.00",
              "~800ms",
              "Large surfaces, ambient settle. Breathes.",
            ],
            [
              "recoil",
              "0.53",
              "~700ms",
              "Toasts, stamps, landings — two bounces. Celebrates.",
            ],
          ]}
        />
        <GuideP>
      ...
      <GuideSection title="Physics for space, tweens for surface">
        <GuideP>
          ...
          rotation. Properties without physical mass (opacity, color, blur, clip
          paths) use the tween scale instead: five durations and four easings,
...
/>`}
        ...
        <GuideP>
          ...
          Exits run as tweens at 0.6× their enter duration with the exit easing,
          which accelerates away instead of landing softly.
        </GuideP>
      </GuideSection>

      <GuideSection title="Why damping ratio is the identity">
        <GuideP>
          Stiffness and damping look independent, but perception collapses them
          into one quality: how the settle feels. That quality is the damping
          ratio, ζ. Two springs with the same ζ at different stiffness are the
          same character at different tempos — which is why the Spring Bench in
          the Playground prints ζ next to everything you try. Tune by
          personality first, tempo second.
        </GuideP>
      </GuideSection>
    </GuideShell>
  );
